...
#' processing time
simCompDataset <- function(dataset, root, numGenes, numCells, seed = 1,
  ...
  tt <- system.time({ # Start timed section
    ...
    #Comment this out when I want the data to match the dimensions of the simulation data
    ...
    if(ncol(counts) > numCells) {
      counts <- counts[, sample(1:ncol(counts), numCells)]
    }
    counts <- counts[rowSums(counts) > 0, ]
    if(nrow(counts) > numGenes) {
      counts <- counts[sample(1:nrow(counts), numGenes), ]
    }
...
compareAllDatasets <- function(sims) {
